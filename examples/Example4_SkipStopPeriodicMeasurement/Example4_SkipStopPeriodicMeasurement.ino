 ...
 *
 * @brief         Skip Stop Periodic Measurement. This example prints the current CO2 level, relative humidity, and
 *                temperature in C.
 *
...
void setup()
{
    ...
    //  readMeasurement; stopPeriodicMeasurement; setAmbientPressure; and getDataReadyStatus. All other commands are
    //  ignored. Because .begin uses getSerialNumber to check if the SCD4x is connected, .begin would fail if a previous
    //  example had left periodic measurements enabled. So, by default, .begin will call stopPeriodicMeasurement before
    //  calling getSerialNumber. The down side to this is that stopPeriodicMeasurement takes 500ms to complete,
    //  incorporating a blocking call of delay(500). If the user is confident that periodic measurements are already
    //  disabled, skipStopPeriodicMeasurements can be set to true to skip the initial call of stopPeriodicMeasurement.
    ...
    {
        Serial.println(
            F("Could not stop periodic measurements. Is the sensor connected? Please check wiring. Freezing..."));
        while (1)
            ;
    }
